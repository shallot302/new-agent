...
import { getState, setProfile, showView, escapeHtml } from './app.js';
...
function updateProgress(): void {
  const bar = document.getElementById('onboarding-progress') as HTMLElement;
...
function prevStep(): void {
  if (currentStep > 1) { currentStep--; renderStep(); updateProgress(); }
}

function nextStep(): void {
  if (currentStep < totalSteps) { currentStep++; renderStep(); updateProgress(); }
}

function renderStep(): void {
  const container = document.getElementById('onboarding-content') as HTMLElement;
...
// Emoji 选择
function initEmojiToggle(): void {
  document.querySelectorAll('.sleep-emoji').forEach(emoji => {
    emoji.addEventListener('click', function(this: HTMLElement) {
      document.querySelectorAll('.sleep-emoji').forEach(e => (e as HTMLElement).style.opacity = '0.4');
      this.style.opacity = '1';
      (this as any)._selected = this.dataset.val;
    });
  });
  document.querySelectorAll('.stress-emoji').forEach(emoji => {
    emoji.addEventListener('click', function(this: HTMLElement) {
      document.querySelectorAll('.stress-emoji').forEach(e => (e as HTMLElement).style.opacity = '0.4');
      this.style.opacity = '1';
      (this as any)._selected = this.dataset.val;
    });
  });
...
function collectFormData(): Record<string, string> {
  ...
  fields.forEach(id => {
    ...
    if (el?.value.trim()) profile[keyMap[id]] = el.value.trim();
  });

  // Radio buttons
  const genderRadio = document.querySelector('input[name="gender"]:checked') as HTMLInputElement;
  if (genderRadio) profile['gender'] = genderRadio.value;

  const trainingRadio = document.querySelector('input[name="training_years"]:checked') as HTMLInputElement;
  if (trainingRadio) profile['training_years'] = trainingRadio.value;

  const skillRadio = document.querySelector('input[name="movement_skill_level"]:checked') as HTMLInputElement;
  if (skillRadio) profile['movement_skill_level'] = skillRadio.value;

  const placeRadio = document.querySelector('input[name="available_place"]:checked') as HTMLInputElement;
  if (placeRadio) profile['available_place'] = placeRadio.value;

  // Chip 多选
  const goals: string[] = [];
  document.querySelectorAll('.goal-chip.bg-blue-600').forEach(c => goals.push((c as HTMLElement).dataset.val || ''));
  if (goals.length) profile['exercise_habits'] = goals.join(', ');

  const healths: string[] = [];
  document.querySelectorAll('.health-chip.bg-blue-600').forEach(c => healths.push((c as HTMLElement).dataset.val || ''));
  if (healths.length) profile['injuries'] = healths.join(', ');

  const equips: string[] = [];
  document.querySelectorAll('.equip-chip.bg-blue-600').forEach(c => equips.push((c as HTMLElement).dataset.val || ''));
  if (equips.length) profile['available_equipment'] = equips.join(', ');

  const occs: string[] = [];
  document.querySelectorAll('.occ-chip.bg-blue-600').forEach(c => occs.push((c as HTMLElement).dataset.val || ''));
  if (occs.length) profile['occupation'] = occs.join(', ');

  const barriers: string[] = [];
  document.querySelectorAll('.barrier-chip.bg-blue-600').forEach(c => barriers.push((c as HTMLElement).dataset.val || ''));
  if (barriers.length) profile['biggest_barrier'] = barriers.join(', ');

  // Emoji
  const sleepEl = document.querySelector('.sleep-emoji[style*="opacity: 1"]') as HTMLElement;
  if (sleepEl && (sleepEl as any)._selected) profile['sleep_quality'] = (sleepEl as any)._selected;

  const stressEl = document.querySelector('.stress-emoji[style*="opacity: 1"]') as HTMLElement;
  if (stressEl && (stressEl as any)._selected) profile['stress_level'] = (stressEl as any)._selected;
  ...
  // 滑块
  const slider = document.getElementById('o-personality_and_motivation') as HTMLInputElement;
  if (slider?.value) profile['personality_and_motivation'] = slider.value;
