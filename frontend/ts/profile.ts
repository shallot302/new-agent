...
function renderProfile(): void {
  const s = getState();
  const container = document.getElementById('profile-display') as HTMLElement;
  const editContainer = document.getElementById('profile-edit') as HTMLElement;
...
function bindEvents(): void {
  document.getElementById('add-profile-field')?.addEventListener('click', async () => {
    const s = getState();
    const key = (document.getElementById('edit-profile-key') as HTMLSelectElement).value;
    const value = (document.getElementById('edit-profile-value') as HTMLInputElement).value.trim();
    if (!key || !value) { alert('请选择字段并填写值'); return; }

    const btn = document.getElementById('add-profile-field') as HTMLButtonElement;
    btn.disabled = true;
    try {
      ...
      renderProfile();
      (document.getElementById('edit-profile-value') as HTMLInputElement).value = '';
    } catch (err) {
